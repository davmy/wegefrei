...
class PhotoLocationExtractorTest {
    ...
    @Test
    fun `extractLocation reads GPS coordinates embedded in EXIF`() =
        runTest {
    ...
    @Test
    fun `extractLocation returns null when the photo has no GPS tags`() =
        runTest {
    ...
    @Test
    fun `extractLocation returns null for a uri that cannot be opened`() =
        runTest {
            val extractor = ExifPhotoLocationExtractor(RuntimeEnvironment.getApplication())
